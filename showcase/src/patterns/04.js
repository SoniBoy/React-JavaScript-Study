...
const MediumClap = ({ children, onClap = (clapState) => {}, style: userStyle = {}, className = "" }) => {
  ...
  return (
    <span>
      <svg
        ...
        className={[styles.icon, isClicked && styles.checked, className].join(" ").trim()}
        style={userStyle}
      >
...
const ClapCount = ({ style: userStyle = {}, className = "" }) => {
  ...
  return (
    <span data-refkey="clapCount" className={[styles.count, className].join(" ").trim()} ref={setRef} style={userStyle}>
      + {count}
    </span>
  );
};
...
const CountTotal = ({ style: userStyle = {}, className = "" }) => {
  ...
  return (
    <span data-refkey="clapTotal" className={[styles.total, className].join(" ").trim()} ref={setRef} style={userStyle}>
      {countTotal}
    </span>
  );
};
...
const Usage = () => {
  ...
  return (
    <>
      <MediumClap onClap={onClap} style={{ border: "1px solid red" }}>
        <ClapIcon />
        {/** MediumClap.Icon*/}
        <ClapCount style={{ background: "blue", color: "black" }} />
        {/** MediumClap.Count*/}
        <CountTotal style={{ background: "blue", color: "black" }} />
        {/** MediumClap.Total*/}
      </MediumClap>
