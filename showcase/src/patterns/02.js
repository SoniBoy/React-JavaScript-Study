...
const MediumClap = () => {
  ...
  return (
    <button className={styles.clap} data-refkey="clap" onClick={onClick} ref={refCallback}>
      ...
      <ClapCount count={state.count} setRef={refCallback} />
      <CountTotal countTotal={state.total} setRef={refCallback} />
    </button>
  );
};
...
const ClapCount = ({ count = 0, setRef }) => {
  return (
...
const CountTotal = ({ countTotal = 250, setRef }) => {
  return (
